...
export async function initLocalServer(port, host, name, rrcsServer) {
	try {
		...
		this.log('error', `Could not initialise Local XML RPC Server: ${e.code ?? JSON.stringify(e) ?? 'Unknown Error'}`)
		this.updateStatus(InstanceStatus.BadConfig, `Could not initialise Local XML RPC Server`)
	}
}
